...
def save_measurement_data(samples):
    ...
    sampling_interval = 0.01  # Adjust based on your actual sampling rate
    timestamps = [round(i * sampling_interval, 2) for i in range(len(samples))]

    with open('power_measurements.csv', 'w', newline='') as csvfile:
    ...
    logger.info("Measurement data saved to 'power_measurements.csv'")
...
def test_power(ppk2, hex_file):
    ...
    averages = []
    all_samples = []  # Initialize list to store all samples
    ...
    while time.time() < start + POWER_TIMEOUT:
        try:
            ...
            if read_data != b'':
                ...
                all_samples.append(average)  # Collect samples
        ...
        save_measurement_data(all_samples)
        generate_time_series_html('power_measurements.csv', 'Time (s)', 'Current (uA)', 'power_measurements_plot.html')
